...
function mousePressed() {
  ...
  if (mouseX >= upperObject.x && mouseX <= upperObject.x + upperObject.width &&
      mouseY >= upperObject.y && mouseY <= upperObject.y + upperObject.height) {
    upperObject.dragging = true;
    dragOffset.x = mouseX - upperObject.x;
    dragOffset.y = mouseY - upperObject.y;
    return;
  }
  
  if (mouseX >= lowerObject.x && mouseX <= lowerObject.x + lowerObject.width &&
      mouseY >= lowerObject.y && mouseY <= lowerObject.y + lowerObject.height) {
    lowerObject.dragging = true;
    dragOffset.x = mouseX - lowerObject.x;
    dragOffset.y = mouseY - lowerObject.y;
    return;
  }
}
...
function mouseDragged() {
  if (upperObject.dragging) {
    upperObject.x = constrain(mouseX - dragOffset.x, 0, width - upperObject.width);
    upperObject.y = constrain(mouseY - dragOffset.y, 0, height - upperObject.height);
  }
  
  if (lowerObject.dragging) {
    lowerObject.x = constrain(mouseX - dragOffset.x, 0, width - lowerObject.width);
    lowerObject.y = constrain(mouseY - dragOffset.y, 0, height - lowerObject.height);
  }
}
