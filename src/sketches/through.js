...
function drawBarrier() {
  ...
  p.fill(barrier.color[0], barrier.color[1], barrier.color[2], 100);
  p.stroke(barrier.color[0], barrier.color[1], barrier.color[2]);
  p.strokeWeight(3);
...
function getDistanceFromBarrier() {
  ...
  let distToCenter = p.dist(movingCircle.x, movingCircle.y, barrierCenterX, barrierCenterY);
...
function drawMovingCircle(isInsideBarrier) {
  ...
  if (isInsideBarrier) {
    p.fill(255, 255, 100);      // Yellow - currently passing through
    p.stroke(255, 200, 0);
    ...
    if (movingCircle.x < barrier.x + barrier.width / 2) {
      p.fill(movingCircle.color[0], movingCircle.color[1], movingCircle.color[2]); // Orange - on left side
      p.stroke(200, 100, 50);
    } else {
      p.fill(100, 255, 100);      // Green - on right side
      p.stroke(50, 200, 50);
    }
  } else {
    p.fill(movingCircle.color[0], movingCircle.color[1], movingCircle.color[2]); // Orange - starting state
    p.stroke(200, 100, 50);
  }
...
function handleInputStart() {
  ...
  if (p.dist(inputX, inputY, movingCircle.x, movingCircle.y) < movingCircle.radius) {
...
function handleInputDrag() {
  if (movingCircle.dragging) {
    ...
    movingCircle.x = p.constrain(movingCircle.x, movingCircle.radius, p.width - movingCircle.radius);
    movingCircle.y = p.constrain(movingCircle.y, movingCircle.radius, p.height - movingCircle.radius);
  }
}
