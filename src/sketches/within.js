 ...
 * • p.dist(), p.rect(), p.ellipse(): P5.js drawing and math functions
 *
 * EXTENSION IDEAS:
...
function drawContainer() {
  ...
  p.fill(container.color[0], container.color[1], container.color[2], 100);
  p.stroke(container.borderColor[0], container.borderColor[1], container.borderColor[2]);
  p.strokeWeight(3);
  p.rect(container.x, container.y, container.width, container.height, 10);
  ...
  p.text(objectsWithin, centerX, centerY);
...
function drawMovingObject() {
  ...
  if (isWithin) {
    p.fill(movingObject.color[0], movingObject.color[1], movingObject.color[2]);
    p.stroke(50, 200, 50); // Green border when within
    p.strokeWeight(3);
  } else {
    p.fill(movingObject.color[0], movingObject.color[1], movingObject.color[2], 150);
    p.stroke(200, 50, 50); // Red border when outside
    p.strokeWeight(2);
  }
  
  p.ellipse(movingObject.x, movingObject.y, movingObject.radius * 2, movingObject.radius * 2);
}

function drawOutsideObject() {
  // This object demonstrates something NOT within the container
  p.fill(outsideObject.color[0], outsideObject.color[1], outsideObject.color[2]);
  p.stroke(100, 100, 100);
  p.strokeWeight(2);
  p.ellipse(outsideObject.x, outsideObject.y, outsideObject.radius * 2, outsideObject.radius * 2);
}
...
function handleInputStart() {
  ...
  let distToMoving = p.dist(inputX, inputY, movingObject.x, movingObject.y);
  let distToOutside = p.dist(inputX, inputY, outsideObject.x, outsideObject.y);
...
function handleInputDrag() {
  ...
  if (movingObject.dragging) {
    ...
    movingObject.x = p.constrain(movingObject.x, movingObject.radius, p.width - movingObject.radius);
    movingObject.y = p.constrain(movingObject.y, movingObject.radius, p.height - movingObject.radius);
  }
  ...
  if (outsideObject.dragging) {
    ...
    // Keep within canvas bounds
    outsideObject.x = p.constrain(outsideObject.x, outsideObject.radius, p.width - outsideObject.radius);
    outsideObject.y = p.constrain(outsideObject.y, outsideObject.radius, p.height - outsideObject.radius);
  }
}
